...
// 转换器Adapter
function Adapter() {
    //数据格式 |消息标识|标识4位（全1表示commond，非全1表示广播来源）|内容4位（标识全1时此处表示飞船ID，否则表示广播飞船状态）|数据8位
    //JSON格式
    //{
    //  type: 'commond',
    //  id: 1,
...
BUS.prototype.broadcast = function(msg) { //发布广播
    var that = this;
    var data = ((this.msgid << 16) | msg);
    var rand = Math.random();
    if (rand < 0.1) { // 广播丢失模拟
        console.log("发送的广播丢失了");
        return;
    }
    this.msgid++;
    // 广播丢失不进行重发
...
function DC() {
    ...
    this.notebook = []; //格式 [shipid, energy, power, lastupdate, state, energynum]
}
DC.prototype.receive = function(msg) {
    var msg = this.adapter.decode(msg);
    var message = JSON.parse(msg);
    if (message.type == 'dc') { // 抛弃非状态广播消息
        var state = this.notebook[message.id];
        state[3] = new Date().getTime();
        state[4] = message.state;
        state[5] = message.energy;
    }
}
DC.prototype.newShip = function(shipid, energyid, powerid) { // 重置信息
    this.notebook[shipid] = [shipid, getEnergyByID(energyid).getName(),
        getPowerByID(
            powerid).getName(), new Date().getTime(), 'N/A', 'N/A'
    ];
}
DC.prototype.update = function(time) { // 更新面板
    for (var i = 1; i <= 4; i++) {
        var dom = dctable[i];
        var trdata = [];
        if (this.notebook[i] == undefined || this.notebook[i] == null) { // 无数据
            for (var j = 0; j < 5; j++) {
                trdata.push("N/A");
                // dom.childNodes[j].innerHTML = 'N/A';
            }
        } else {
            var lastupdate = this.notebook[i][3];
            if ((new Date().getTime()) - lastupdate > 2000) { // 两秒未广播数据，可能已经销毁
                for (var j = 0; j < 5; j++) {
                    trdata.push("N/A");
                    // dom.childNodes[j].innerHTML = 'N/A';
                }
            } else {
                var data = this.notebook[i];
                trdata[0] = data[0];
                trdata[1] = data[1];
                trdata[2] = data[2];
                trdata[3] = flystate(data[4]);
                trdata[4] = data[5] + "%";
                // dom.childNodes[1].innerHTML = data[0];
                // dom.childNodes[3].innerHTML = data[1];
                // dom.childNodes[5].innerHTML = data[2];
                // dom.childNodes[7].innerHTML = data[4];
                // dom.childNodes[9].innerHTML = data[5];
                // console.log(data);
            }
        }
        dom.innerHTML = '<td>' + trdata.join('</td><td>') + '</td>';
    }
}
...
Ship.prototype.update = function(time) {
    ...
    this.delaytime += time; //累积时间
    if (this.delaytime >= 1000) { //累积1000ms发送广播
        this.delaytime -= 1000;
        var message = {};
        message.type = 'dc';
        message.id = this.id;
        if (this.isdestory) {
            message.state = 'destory';
        } else {
            if (this.isrunning) {
                message.state = 'start';
            } else {
        ...
        message.energy = this.energy;
        mediator.broadcast(this.adapter.encode(JSON.stringify(message))); // 广播状态消息
    }
}
...
Ship.prototype.receive = function(msg) { // 接收消息
    ...
    if (message.type != 'commond') { // 丢弃非指令消息
        return;
    }
...
PowerA1.prototype.getName = function() {
    return "前进号";
}
...
PowerA2.prototype.getName = function() {
    return "奔腾号";
}
...
PowerA3.prototype.getName = function() {
    return "超越号";
}
...
EnergyB1.prototype.getName = function() {
    return "劲能型";
}
...
EnergyB2.prototype.getName = function() {
    return "光能型";
}
...
EnergyB3.prototype.getName = function() {
    return "永久型";
}
...
Commonder.prototype.commondNewShip = function(energyid, powerid) {
    ...
    dc.newShip(shipid, energyid, powerid);
